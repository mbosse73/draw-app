...
export function SettingsMenu() {
  ...
  const handleExport = () => {
    ...
    setOpen(false);
    showToast("Einstellungen heruntergeladen", "success");
  };
  ...
  const handleImport = async () => {
    ...
    if (!result.success) {
      showToast("Einstellungen konnten nicht geladen werden: " + result.error, "error");
      return;
    }
    setOpen(false);
    showToast(`Einstellungen importiert (${result.favoriteCount} Favoriten)`, "success");
  };

  return (
    <div className="toolbar-menu" ref={menuRef}>
      <button onClick={() => setOpen((o) => !o)} className="icon-btn" title="Einstellungen (z.B. Favoriten) sichern/laden" aria-label="Einstellungen">
        ⚙
      </button>
      {open && (
