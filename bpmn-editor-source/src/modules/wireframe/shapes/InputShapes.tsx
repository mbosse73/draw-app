  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      ...
      {Array.from({ length: lineCount }).map((_, i) => (
        <SketchPaths key={i} paths={sketchLine(8, 14 + i * 16, w - 8 - (i === lineCount - 1 ? w * 0.35 : 0), 14 + i * 16, seedFor(shape.id, `l${i}`), { stroke: WIREFRAME_COLORS.accentFill, strokeWidth: 2 })} />
      ))}
    </g>
  );
}
...
function CheckboxRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths paths={sketchRoundedRect(box, box, seedFor(shape.id), { stroke })} />
      {checked && <SketchPaths paths={sketchPath(`M ${box * 0.2} ${box * 0.55} L ${box * 0.42} ${box * 0.78} L ${box * 0.82} ${box * 0.22}`, seedFor(shape.id, "check"), { stroke, strokeWidth: 1.8 })} />}
...
function RadioRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths paths={sketchCircle(d / 2, h / 2, d, seedFor(shape.id), { stroke })} />
      {checked && <SketchPaths paths={sketchCircle(d / 2, h / 2, d * 0.4, seedFor(shape.id, "dot"), { stroke, fill: stroke, fillStyle: "solid", roughness: 0.4 })} />}
...
function ComboboxRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths paths={sketchRoundedRect(w, h, seedFor(shape.id), { stroke, fill: "#ffffff" })} />
      <SketchPaths paths={sketchPath(`M ${w - 22} ${h / 2 - 3} L ${w - 14} ${h / 2 + 4} L ${w - 6} ${h / 2 - 3}`, seedFor(shape.id, "chev"), { stroke })} />
...
function SliderRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths paths={sketchLine(6, h / 2, w - 6, h / 2, seedFor(shape.id), { stroke, strokeWidth: 1.8 })} />
      <SketchPaths paths={sketchCircle(handleX, h / 2, 14, seedFor(shape.id, "handle"), { stroke, fill: "#ffffff" })} />
    </g>
  );
}
...
function SpinnerRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths paths={sketchRoundedRect(w, h, seedFor(shape.id), { stroke, fill: "#ffffff" })} />
      <SketchPaths paths={sketchLine(w - 20, 0, w - 20, h, seedFor(shape.id, "div"), { stroke, strokeWidth: 0.8 })} />
      <SketchPaths paths={sketchPath(`M ${w - 15} ${h / 2 - 1} L ${w - 10} ${h / 2 - 6} L ${w - 5} ${h / 2 - 1}`, seedFor(shape.id, "up"), { stroke })} />
      <SketchPaths paths={sketchPath(`M ${w - 15} ${h / 2 + 1} L ${w - 10} ${h / 2 + 6} L ${w - 5} ${h / 2 + 1}`, seedFor(shape.id, "down"), { stroke })} />
...
function SearchFieldRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths paths={sketchRoundedRect(w, h, seedFor(shape.id), { stroke, fill: "#ffffff" })} />
      <SketchPaths paths={sketchCircle(16, h / 2 - 1, 9, seedFor(shape.id, "lens"), { stroke })} />
      <SketchPaths paths={sketchLine(20, h / 2 + 3, 24, h / 2 + 7, seedFor(shape.id, "handle"), { stroke })} />
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      ...
      <g transform={`translate(${iconX} ${h / 2 - 7})`}>
        <SketchPaths paths={sketchRoundedRect(16, 14, seedFor(shape.id, "cal"), { stroke }, 2)} />
        <SketchPaths paths={sketchLine(0, 4.5, 16, 4.5, seedFor(shape.id, "calbar"), { stroke, strokeWidth: 1 })} />
        <SketchPaths paths={sketchLine(4, 0, 4, 3, seedFor(shape.id, "ring1"), { stroke, strokeWidth: 1.4 })} />
        <SketchPaths paths={sketchLine(12, 0, 12, 3, seedFor(shape.id, "ring2"), { stroke, strokeWidth: 1.4 })} />
      </g>
