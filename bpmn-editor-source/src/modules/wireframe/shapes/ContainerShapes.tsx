...
function PanelRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths shape={shape} paths={sketchRoundedRect(w, h, seed, { stroke }, 4)} />
...
function TabContainerRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths shape={shape} paths={sketchRect(w, h - TAB_HEIGHT, seedFor(shape.id, "body"), { stroke })} />
      {tabs.map((tab, i) => (
        <g key={i} transform={`translate(${i * TAB_WIDTH} 0)`}>
          <SketchPaths shape={shape} paths={sketchRect(TAB_WIDTH, TAB_HEIGHT, seedFor(shape.id, `tab${i}`), { stroke })} />
...
function SplitterRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths shape={shape} paths={sketchLine(w / 2, 0, w / 2, h, seedFor(shape.id), { stroke, strokeWidth: 1.6 })} />
    </g>
  );
}
...
function ScrollbarRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <SketchPaths shape={shape} paths={sketchRect(w, h, seedFor(shape.id), { stroke, fill: WIREFRAME_COLORS.fillLight })} />
      {isVertical ? (
        <SketchPaths shape={shape} paths={sketchRect(w - 4, thumbLength, seedFor(shape.id, "thumb"), { stroke, fill: WIREFRAME_COLORS.accentFill, fillStyle: "solid" })} />
      ) : (
        <SketchPaths shape={shape} paths={sketchRect(thumbLength, h - 4, seedFor(shape.id, "thumb"), { stroke, fill: WIREFRAME_COLORS.accentFill, fillStyle: "solid" })} />
      )}
    </g>
  );
}
    ...
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      {rows.map((row, i) => (
        <g key={i} transform={`translate(0 ${row.y})`}>
          <SketchPaths shape={shape} paths={sketchRect(w, ACCORDION_HEADER_H, seedFor(shape.id, `hdr${i}`), { stroke, fill: WIREFRAME_COLORS.fillLight })} />
          ...
          {row.expanded && (
            <g transform={`translate(0 ${ACCORDION_HEADER_H})`}>
              <SketchPaths shape={shape} paths={sketchRect(w, bodyHeight, seedFor(shape.id, "body"), { stroke })} />
            </g>
          )}
        </g>
      ))}
