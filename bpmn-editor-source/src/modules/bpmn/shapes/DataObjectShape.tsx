...
import { DATA_OBJECT_SIZE, BPMN_COLORS, resolveFill, resolveStroke, resolveStrokeWidth, resolveDashArray } from "./constants";
...
function DataObjectRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <path d={outline} fill={resolveFill(shape, BPMN_COLORS.fill)} stroke={stroke} strokeWidth={strokeWidth} strokeDasharray={resolveDashArray(shape)} />
      <path d={foldLine} fill="none" stroke={stroke} strokeWidth={strokeWidth} />
      {shape.data.label ? (
        <MultilineText
