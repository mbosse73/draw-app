...
import { BPMN_COLORS } from "./constants";
...
function PoolRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      <rect width={w} height={h} fill="#ffffff" stroke={stroke} strokeWidth={isSelected ? 2 : 1.5} />
...
function LaneRender({ shape, isSelected }: ShapeRenderProps) {
  ...
  return (
    <g transform={`translate(${shape.position.x} ${shape.position.y})`}>
      ...
      <rect width={w} height={h} fill="rgba(255,255,255,0.4)" stroke={stroke} strokeWidth={isSelected ? 2 : 1} strokeDasharray={isSelected ? undefined : "0"} />
